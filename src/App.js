...
const App = () => {
  ...
  // true because upon click the edit button we have to pass the value true
  ...
  const handleDelete = (id) => {
    ...
    // set updated task in UI / Real time updated UI
    setTasks(tasks.filter((task) => task.id !== id));
  };

  const deleteData = async (id) => {
    await fetch(`https://necessary-jumpy-celery.glitch.me/tasks/${id}`, {
      method: "DELETE",
      headers: {
  ...
  // Editing event
  const handleEdit = (id) => {
    // ektate click kore unfinished rekhe abar arektate click kora jay....so duita element add hoiteche
